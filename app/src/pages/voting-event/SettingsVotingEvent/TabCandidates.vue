...
import {
  Dialog,
  QBtn, QImg, QInput, QSeparator, QTab, QTableColumn,
  QTabPanel,
  QTabPanels,
  QTabs,
} from 'quasar';
    ...
    fields: [
      {
        ...
        field: () => [
          ...
          h('div', {
            ...
            default: ({ state: [descTab, setDescTab] }: any) => [
              h(QTabPanels, {
                modelValue: descTab,
                vertical: true,
                class: 'flex-grow',
                'onUpdate:modelValue': setDescTab,
              }, () => [
                h(QTabPanel, {
                  name: 'edit',
                  class: 'py-0 pl-0',
                  ...
                  label: 'Deskripsi',
                  type: 'textarea',
                  modelValue: candidateFields.value.desc,
                  placeholder: 'Masukkan Visi dan Misi',
                  'onUpdate:modelValue': (val: string) => { candidateFields.value.desc = val; },
                  hint: 'Menggunakan format Markdown, referensi selengkapnya: https://www.markdownguide.org/cheat-sheet/',
                  autogrow: true,
                  filled: true,
                })),
                h(QTabPanel, {
                  ...
                  innerHTML: MarkdownIt().render(candidateFields.value.desc),
                  class: 'prose',
                })),
              ]),
              h(QSeparator, { vertical: true }),
              h(QTabs, {
                modelValue: descTab,
                vertical: true,
...
const onEditClick = (item: Votable) => {
  ...
  showCandidateDialog('Lihat/Perbarui Kandidat')
    .onOk(() => showTheLoadingAndNotifyErrorAsync(
      () => votable.update({
        desc: candidateFields.value.desc,
        subtitle: candidateFields.value.subtitle,
        title: candidateFields.value.title,
        img: candidateFields.value.img !== item.thumbnailSrc
